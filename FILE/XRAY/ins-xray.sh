...
sed -i '$ ilocation /onexd {' /etc/nginx/conf.d/xray.conf
sed -i '$ iif ($http_upgrade != "Upgrade") {' /etc/nginx/conf.d/xray.conf
sed -i '$ irewrite /(.*) /onexd break;' /etc/nginx/conf.d/xray.conf
sed -i '$ i     }' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_redirect off;' /etc/nginx/conf.d/xray.conf
...
wget -q -O /usr/bin/bbr "https://raw.githubusercontent.com/onexd77/sange/main/FILE/SSH/bbr.sh"
chmod +x /usr/bin/bbr
bbr >/dev/null 2>&1
...
sleep 1
wget -q -O /usr/bin/auto-set "https://raw.githubusercontent.com/onexd77/sange/main/FILE/XRAY/auto-set.sh" && chmod +x /usr/bin/auto-set 
wget -q -O /usr/bin/crtxray "https://raw.githubusercontent.com/onexd77/sange/main/FILE/XRAY/crt.sh" && chmod +x /usr/bin/crtxray 
sleep 1
